...
describe("SwapV3-test", function () {
    ...
    beforeEach(async function () {
        ...
        let FakeUSDCFactory = await ethers.getContractFactory("MockUSDCMintable");
        fakeUSDCContract = await FakeUSDCFactory.deploy();

        let FakeUSDTFactory = await ethers.getContractFactory("MockUSDTMintable");
        ...
        const INIT = await deployer.deploy(
            swapflashloan.address,
            [fakeDAIContract.address, fakeUSDCContract.address, fakeUSDTContract.address],
            [18, 6, 6],
            "FSS-FAKE-DAI-USDC-USDT-V3", // name
            "LP-FAKE-USD-V3", // symbol
            100, // a
            100, // fee
            100, // adminFee
            lptoken.address, // lpToken
            lprewards.address // lpReward
        );
        ...
        const INIT_STANDALONE = await swapflashloan.initialize(
            [fakeDAIContract.address, fakeUSDCContract.address, fakeUSDTContract.address],
            [18, 6, 6],
            "FSS-FAKE-DAI-USDC-USDT-V3",
            "LP-FAKE-USD-V3",
            100,
            10000000,
            1000000000,
        ...
        const DEPOSIT = await swapflashloan.addLiquidity(
            ["100000000000000000000","100000000","100000000"],
            1,
            TIMESTAMP_2121
        );
