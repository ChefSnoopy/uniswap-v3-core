...
library SwapUtilsV3 {
    ...
    function _getAPrecise(Swap storage self) internal view returns (uint256) {
        return AmplificationUtilsV3._getAPrecise(self);
    }
    ...
    function getYD(
        ...
        c = c.mul(d).mul(AmplificationUtilsV3.A_PRECISION).div(
            nA.mul(numTokens)
        );

        uint256 b = s.add(d.mul(AmplificationUtilsV3.A_PRECISION).div(nA));
        uint256 yPrev;
        uint256 y = d;
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < MAX_LOOP_LIMIT; i++) {
            ...
            d = nA
            .mul(s)
            .div(AmplificationUtilsV3.A_PRECISION)
            .add(dP.mul(numTokens))
            .mul(d)
            .div(
                nA
                .sub(AmplificationUtilsV3.A_PRECISION)
                .mul(d)
                .div(AmplificationUtilsV3.A_PRECISION)
        ...
        returns (uint256)
    {
        ...
        LPTokenV3 lpToken = self.lpToken;
